...
import { ParkingLotsContainer, ParkingLot, ReservedParkingTable, ReservedParkingTableHeader, ReservedParkingTableCell } from "./ParkingReservation.style";

const ParkingReservation: React.FC = () => {
    const [selectedDateTime, setSelectedDateTime] = useState<Date | null>(new Date());
    const [selectedSpace, setSelectedSpace] = useState<number | null>(null);
    const [reservedParking, setReservedParking] = useState<Record<string, number[]>>({});
    ...
    const handleSpaceSelection = (spaceNumber: number) => {
        setSelectedSpace((prevSelectedSpace) => (prevSelectedSpace === spaceNumber ? null : spaceNumber));
    };
    ...
    const handleReservation = () => {
        if (selectedSpace !== null && selectedDateTime !== null) {
            ...
            console.warn("Please select a parking space and date before making a reservation.");
        }
    };
    ...
    return (
        <div>
            ...
            <div>
                <DatePicker selected={selectedDateTime} onChange={handleDateTimeChange} showTimeSelect timeFormat="HH:mm" timeIntervals={60} dateFormat="MMMM d, yyyy h:mm aa" />
            </div>
            <ParkingLotsContainer>
                {[...Array(10)].map((_, index) => (
                    <ParkingLot
                        ...
                        style={{
                            backgroundColor:
                                selectedSpace === index + 1 ? "blue" : reservedParking[selectedDateTime?.toISOString().split("T")[0]]?.includes(index + 1) ? "red" : "transparent",
                        }}
            ...
            <div>
                <button onClick={handleReservation} disabled={selectedSpace === null || selectedDateTime === null}>
            ...
            <ReservedParkingTable>
                <thead>
                    <tr>
                        ...
                        <ReservedParkingTableHeader>Reserved Parking Lots</ReservedParkingTableHeader>
                    </tr>
                </thead>
                <tbody>
                    {Object.entries(reservedParking).map(([date, parkingLots]) => (
                        <tr key={date}>
                            ...
                            <ReservedParkingTableCell>{parkingLots.join(", ")}</ReservedParkingTableCell>
                        </tr>
                    ))}
                </tbody>
            </ReservedParkingTable>
